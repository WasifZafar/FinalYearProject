...
const ElectoralRegister = () => {
  ...
  const handleAadhaarCheck = async () => {
    ...
    try {
      const electorsRef = collection(db, "electors");
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try {
      const { dob, email, surnamePrefix, firstName, lastName } = formData;
      ...
      await addDoc(collection(db, "electors"), {
        aadhaar,
        name: combinedName,
        dob,
  ...
  return (
    ...
    {!aadhaarValidated ? (
      <div className="aadhaar-container">
        <h2 className="heading">Register in Electoral Roll..</h2>
        <h3 >Check Aadhaar</h3>
        <input
          type="text"
...
export default ElectoralRegister;
